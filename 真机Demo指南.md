...
> **公网模式（推荐，队友直接测）**：Mock Server 已部署到云服务器——`http://140.143.207.74:8100`（systemd 常驻 + 开机自启），装「Avenlo-公网测试版.apk」即可，**不依赖任何人的电脑**。本指南其余部分是电脑本地模式的备用方案。

## 准备清单
- 真机一台（Android 8.0+）
- 公网模式：什么都不用（服务器已在跑）；本地模式：与电脑**同一 WiFi** + 电脑端 Mock Server 已启动
- 备份兜底：现场网络全挂时播放 `emulator-screens/demo_backup.mp4`（v7，252s，含全部新动线——弹层/关联跳转/灵感集/今日回顾/我的页）
...
6. 点卡片进详情页 → AI 摘要 + 相关想法（可点击跳转其详情）+ 参考资源；**详情页下滑到相关想法区，点「共N条 ›」展开完整列表弹层**（第四十一轮）——每条关联带**关联原因胶囊**（相似主题/同灵感集），点任意关联卡直接跳其详情。「为什么关联」的可解释性是 Q&A 加分项
...
11. 详情页「参考资源」条目与「立即全部 ›」、今日回顾「明日待延展」条目均可点击（Snackbar 反馈，第四十一轮）——全 App 无静默死点
12. **撤销演示（仅真机可演示，第三十一轮）**：录完音保存后 **3 秒内双捏（快速点两次 FAB）**→ 卡片被撤销删除、不提交 server。模拟器自动化测不了这条（dump 耗时 2-3s 吃掉撤销窗），现场真机演示是它唯一的展示途径——值得专门排练一次
...
| **断网时捕捉的卡** | server 不可达（WiFi 掉线/防火墙） | 卡以「整理中」占位保留（不丢），**网络恢复后 15s 内自动补交为完整卡**（第四十轮重试队列）；唯一边界：断网期间 App 进程被杀则队列丢失，需删占位卡重录 |
